...
def label_smooth_cross_entropy(pred, label, weight=None, reduction='mean',
                               class_weight=None, avg_factor=None, smoothing=0.0):
    confidence = 1.0 - smoothing
    label_shape = torch.Size((label.size(0), pred.size(1))) # N,6
    pred = pred.log_softmax(-1)
    with torch.no_grad():
        true_dist = torch.zeros_like(pred)
        true_dist.fill_(smoothing / (pred.size(1) - 1)) # s / (C-1)
