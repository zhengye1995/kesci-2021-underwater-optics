...
@PIPELINES.register_module()
class LoadImageFromFileWithTemplate(object):
    """Load an image and template from file.
    ...
    def __call__(self, results):
        ...
        template_filename = osp.basename(filename).split('.')[0] + '_t' + '.jpg'
        if not isinstance(self.template_path, list):
            ...
            if "20210204" in filename:
                template_filename = osp.join(self.template_path[0], template_filename)
            elif "20210208" in filename:
                template_filename = osp.join(self.template_path[1], template_filename)
            else:
...
@PIPELINES.register_module()
class LoadImageWithTemplateFromWebcam(LoadImageFromFileWithTemplate):
    """Load an image from webcam.
